  ...
  if (adj)
    {
      ...
      txid = (vnet_get_sup_hw_interface (&vnet_main, txid))->hw_if_index; //VAR OU APPEL DE FONCTION POUR VNET?
      // Get the device associated with the hardware index provided
      vnet_hw_interface_t *hw = vnet_get_hw_interface (&vnet_main, txid); //IDEM
      if (hw)
	{
	  dpdk_device_t *txd =
	  ...
	  if (txd)
	    {
	      u16 ring_size = txd->conf.n_tx_desc; //MODIF
	      u16 tx_port_id = txd->port_id;

	      u16 tx_queue_id = vlib_get_main()->thread_index % txd->conf.n_tx_queues; //MODIF
	      u16 num_desc = 0;
